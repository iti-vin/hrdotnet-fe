...
export default function Drawer({ ...props }: Drawer) {
  const { opened, onClose, position, size, title = "Sample Title", children, footer, contentClassName, bodyClassName, headerClassName, footerClassName } = props;
  return (
    <DrawerComponent styles={{ body: { height: "100%" } }} opened={opened} onClose={onClose} position={position} size={size} withCloseButton={false} p={0}>
      <div className={cn(drawer.contentClass, contentClassName)}>
        <div className="flex flex-col gap-2 2xl:gap-4 p-0">
          ...
          <div className={cn(drawer.bodyClass, bodyClassName)}>
            {/* Divider */}
            {children}
            <div className="w-full h-[10px] bg-[#edeeed] animate-pulse" />
            <div className="w-full h-[10px] bg-[#edeeed] animate-pulse" />
            <div className="w-full h-[10px] bg-[#edeeed] animate-pulse" />
          </div>
        </div>
