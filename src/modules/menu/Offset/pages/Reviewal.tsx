...
export default function Reviewal() {
  ...
  const { data, isLoading, refetch } = useQuery<OffsetResponse>({
    queryKey: ["reviewal_offset", { ...storedFilters }, { ...storedPage }],
    queryFn: async () => {
      const startTime = performance.now();
      const result = await OffsetServices.getAllForReviewalOFF({ ...storedFilters, ...storedPage });
  ...
  const { mutate: singleEndorse } = useMutation({
    mutationFn: async (id: number) => {
      ...
      return OffsetServices.singleEndorseOFF(id, formData);
    },
    onSuccess: () => {
      setOpenDialog("");
