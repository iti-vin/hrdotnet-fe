...
import { DateRangePickerInput, MultiSelect, TextInput, Button, Drawer } from "@shared/components";
...
export default function DrawerFilter({ isNotUser = false }: DrawerFilterProps) {
  ...
  const { openDrawer, setOpenDrawer } = useOvertimeStore();
  const { onHandleSubmitFilter, onHandleClearFilter } = useOvertimeContext();
      ...
      <div className="w-full h-full flex flex-col gap justify-between">
        <div className="flex flex-col gap-2 2xl:gap-4">
          <Flex className="flex flex-col gap-2">
            ...
            <MultiSelect label="Schedule" radius="md" classNames={{ input: "poppins" }} data={[]} styles={{ label: { color: "#6d6d6d", fontSize: "15px" } }} />
            <Divider size={2} h={10} color="#edeeed" className="w-full" />
              ...
              <Tabs.Panel value="missedlog">
                <Group>
                  ...
                  <DateRangePickerInput
                    ...
                    sp="To"
                    dateValue={dateRange}
                    setDateValue={(dateRange: [string | null, string | null]) => {
