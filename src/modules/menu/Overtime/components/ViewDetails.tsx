...
import { IconCalendar, IconChevronDown, IconDots, IconNotes } from "@tabler/icons-react";
import { Modal } from "@shared/template/";

import { Container, Flex, Group, MultiSelect, Select, Text, Textarea, TextInput, useMatches } from "@mantine/core";
...
import { Buttons } from "@/modules/menu/Overtime/components/Button";
import { useDisclosure } from "@mantine/hooks";
import { Alerts, Rejected } from "@/modules/menu/Overtime/components/AlertOT";
            ...
            <div className="flex flex-col gap-2">
              ...
              <Flex direction={{ base: "column", sm: "row" }} justify="space-between" className="w-full" gap={20}>
                <Select
                  ...
                  disabled
                />
                <TextInput size="lg" radius={8} label="Reference No." placeholder="0000-0000-0000" className="w-full" disabled />
              </Flex>
              ...
              <div className="flex flex-col gap-8  sm:gap-9">
                <Flex direction={{ base: "column", sm: "row" }} justify="space-between" className="w-full" gap={20}>
                  ...
                  <TimeInput
                    ...
                    radius={8}
                    label="Actual OT Out"
                    value={DateTimeUtils.getIsoTimeDefaultWithUnits(selectedData.actualFrom)}
                    placeholder={DateTimeUtils.getIsoTimeDefaultWithUnits(selectedData.actualFrom)}
                    className="w-full"
                    disabled
                    styles={{ label: { color: "#6d6d6d", fontSize: "15px" } }}
                  />
                ...
                <Flex direction={{ base: "column", sm: "row" }} justify="space-between" className="w-full" gap={20}>
                  <TimeInput size="lg" radius={8} label="OT From" className="w-full" disabled styles={{ label: { color: "#6d6d6d", fontSize: "15px" } }} />
                  <TimeInput size="lg" radius={8} label="OT To" className="w-full" disabled styles={{ label: { color: "#6d6d6d", fontSize: "15px" } }} />
                </Flex>
                <DatePickerInput valueFormat="YYYY MMM DD" label="Duration" placeholder="Pick date" className="w-full" size="lg" radius={8} disabled />
              </div>
            </div>
          </div>
          ...
          <div className="w-full md:w-1/2 flex flex-col gap-2 border-solid border-0.5 border-sky-500 p-4 rounded-lg">
            ...
            <div className="flex flex-col md:flex-row justify-between gap-4">
              <div className="flex flex-col w-full md:w-1/2">
                ...
                <TextInput disabled={isReadOnly} label="Document No." radius="md" size="lg" placeholder="00000000" />
              </div>

              <div className="flex flex-col w-full md:w-1/2">
                {/* <Text >Transaction Date</Text> */}
                <DatePickerInput disabled={isReadOnly} label="Transaction Date" radius="md" size="lg" placeholder="mm/dd/yyyy" />
              </div>
            </div>
            ...
            <div className="flex flex-col">
              ...
              <Textarea disabled={isReadOnly} label="Cancellation Information" size="lg" radius="md" placeholder="No Information" className="w-full" />
            </div>
          </div>
        </Flex>
