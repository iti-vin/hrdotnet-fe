...
import { useML } from "@/modules/menu/MissedLog/store/useMissedLog";
// Shared Modules
import { FilingStatus } from "@shared/assets/types/Global";
import { Header, Container, StatusChip, Filter } from "@shared/template";
// Own Modules
import { Table, DrawerFilter, Details } from "@/modules/menu/MissedLog/components/views";
...
export default function Filings() {
  ...
  const { setActiveTab, setAction } = useML();
  ...
  return (
    <Container>
      <Header title="Missed Log" buttonLabel="Approve" buttonIcon={<IconCircleCheck size={25} stroke={2} />} buttonClick={() => setAction("Approve")} />
      <Filter filterOpen={filterOpen} />

      <Table
        statuses={[FilingStatus.Filed, FilingStatus.Reviewed, FilingStatus.Approved]}
        columns={[
      ...
      <Details tabs="Approve" opened={details} onClose={detailsClose} buttonClose={detailsClose} />
    </Container>
  );
}
