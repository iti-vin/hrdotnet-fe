...
export default function Reviewal() {
  ...
  const { data, isLoading, refetch } = useQuery<ChangeSchedule>({
    queryKey: ["reviewal_cos", { ...storedFilters }, { ...storedPage }],
    queryFn: async () => {
      const startTime = performance.now();
      const result = await CosServices.getAllCosReviewal({ ...storedFilters, ...storedPage });
  ...
  const { mutate: singleEndorse } = useMutation({
    mutationFn: async (id: number) => {
      ...
      return CosServices.singleEndorseCOS(id, formData);
    },
    onSuccess: () => {
  ...
  return (
    <Container>
      ...
      <Modals panel={panel} onHandleSingleEndorse={() => singleEndorse(singleItem.filing.id)} />
    </Container>
  );
}
