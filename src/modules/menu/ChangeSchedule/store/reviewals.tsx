...
type Type = {
  ...
  onBatchEndorse: boolean;
  setOnBatchEndorse(batch: boolean): void;
  onBatchCancel: boolean;
  setOnBatchCancel(batch: boolean): void;
};

const useReviewalCosStore = create<Type>((set) => ({
  // --- Fetch Arrays
  ...
  // --- Modal
  onBatchEndorse: false,
  setOnBatchEndorse: (batch: boolean) => set({ onBatchEndorse: batch }),
  onBatchCancel: false,
  setOnBatchCancel: (batch: boolean) => set({ onBatchCancel: batch }),
}));

export default useReviewalCosStore;
