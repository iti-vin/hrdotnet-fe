...
const leaveTypes = [
  ...
  { count: 1, label: "SSS Allocation Leave", icon: <p style={{ fontSize: "2.6rem", color: "white", fontWeight: "600", width: "4rem", textAlign: "center" }}>SSS</p> },
    ...
    <div className="bg-white m-4  -mb-16 lg:h-screen-85 overflow-y-hidden -z-10 p-8 rounded-lg bottom-0">
      ...
      <div className="flex flex-col sm:grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
        {leaveTypes.map((leave, index) => (
          <div
            key={leave.label}
            className="cursor-pointer bg-white h-56 lg:p-2 flex flex-col items-center justify-center shadow-md shadow-blue-300 rounded-lg"
            onClick={() => {
            ...
            <div className="flex items-center justify-start mt-2 -ml-16 max-w-16 gap-6">
              ...
              <Text color="#767F8C">{leave.label}</Text>
            </div>
          </div>
        ))}
      </div>
