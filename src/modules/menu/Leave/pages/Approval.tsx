...
export default function Approval() {
  ...
  const { time, setTime, viewItems, setOpenDialog, setError, loading, setLoading, storedFilters, storedPage } = useLeaveStore();
  ...
  const { mutate: singleApprove } = useMutation({
    mutationFn: async (id: number) => {
      ...
      return LeaveServices.singleApproveLeave(id, formData);
    },
    onSuccess: () => {
  ...
  const onHandleSingleApprove = () => {
    singleApprove(viewItems.filing.id);
  };
