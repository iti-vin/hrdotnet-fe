...
import { PaginationI } from "@shared/assets/types/Component";
export default function index({ total, pageSize, recordsLength, time, currentPage }: PaginationI) {
  const isMobile = useMediaQuery(`(max-width: ${em(750)})`);

  return (
    <Group justify={isMobile ? "center" : "space-between"} align="center" className="h-[5%] items-end">
      <Flex>
        <Text size="sm" className="text-gray-500">
          ...
          <select
            ...
            onChange={(e) => console.log({ PageSize: e.target.value })}>
      ...
      <Pagination onChange={undefined} total={total} size="sm" value={undefined} />
    </Group>
  );
}
